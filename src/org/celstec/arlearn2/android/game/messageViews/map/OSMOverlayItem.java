...
public class OSMOverlayItem extends OverlayItem{
    ...
    if (drawableHashMap.containsKey(gi.getType())) return drawableHashMap.get(gi.getType());
    Bitmap bitmapNoIcon = BitmapFactory.decodeResource(ctx.getResources(), R.drawable.game_general_item_marker_noicon);
    Bitmap icon = BitmapFactory.decodeResource(ctx.getResources(), GeneralItemMapper.mapConstantToDrawable(GeneralItemMapper.mapBeanToConstant(gi)));
