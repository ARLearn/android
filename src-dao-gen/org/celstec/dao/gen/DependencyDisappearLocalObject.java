...
// KEEP INCLUDES END
/**
 * Entity mapped to table DEPENDENCY_DISAPPEAR_LOCAL_OBJECT.
 */
public class DependencyDisappearLocalObject {
    ...
    private transient DependencyDisappearLocalObjectDao myDao;

    private GeneralItemLocalObject generalItemLocalObject;
    private Long generalItemLocalObject__resolvedKey;

    private List<DependencyDisappearLocalObject> childDeps;

    // KEEP FIELDS - put your custom fields here
    // KEEP FIELDS END

    public DependencyDisappearLocalObject() {
    }

    public DependencyDisappearLocalObject(Long id) {
        this.id = id;
    }

    public DependencyDisappearLocalObject(Long id, Integer type, String action, Integer scope, Long timeDelta, Long radius, Double lat, Double lng, Long parentDependency, Long generalItemId) {
        this.id = id;
        this.type = type;
        this.action = action;
        this.scope = scope;
    ...
    public void __setDaoSession(DaoSession daoSession) {
        ...
        myDao = daoSession != null ? daoSession.getDependencyDisappearLocalObjectDao() : null;
    }
    ...
    public List<DependencyDisappearLocalObject> getChildDeps() {
        if (childDeps == null) {
            if (daoSession == null) {
                throw new DaoException("Entity is detached from DAO context");
            }
            DependencyDisappearLocalObjectDao targetDao = daoSession.getDependencyDisappearLocalObjectDao();
            List<DependencyDisappearLocalObject> childDepsNew = targetDao._queryDependencyDisappearLocalObject_ChildDeps(id);
            synchronized (this) {
                if(childDeps == null) {
    ...
    private String toStringAnd() {
        ...
        for (DependencyDisappearLocalObject dependencyLocalObject: this.getChildDeps()) {
    ...
    public DependencyDisappearLocalObject(ActionDependency dependsOn) {
        ...
        DaoConfiguration.getInstance().getDependencyDisappearLocalObjectDao().insertOrReplace(this);
    }

    public DependencyDisappearLocalObject(TimeDependency dependsOn) {
        setType(TIME_DEPENDENCY);
        setTimeDelta(dependsOn.getTimeDelta());
        DaoConfiguration.getInstance().getDependencyDisappearLocalObjectDao().insertOrReplace(this);
        if (dependsOn.getOffset()!= null) {
            DependencyDisappearLocalObject localObject = DependencyDisappearLocalObject.createDependencyDisappearLocalObject(dependsOn.getOffset());
            localObject.setParentDependency(this.getId());
            DaoConfiguration.getInstance().getDependencyDisappearLocalObjectDao().insertOrReplace(localObject);
        }
    }

    public DependencyDisappearLocalObject(ProximityDependency dependsOn) {
    ...
    public DependencyDisappearLocalObject(AndDependency dependencyBean) {
        setType(AND_DEPENDENCY);
        DaoConfiguration.getInstance().getDependencyDisappearLocalObjectDao().insertOrReplace(this);
        for (Dependency dep: dependencyBean.getDependencies()){
            DependencyDisappearLocalObject localObject = DependencyDisappearLocalObject.createDependencyDisappearLocalObject(dep);
            localObject.setParentDependency(this.getId());
            DaoConfiguration.getInstance().getDependencyDisappearLocalObjectDao().insertOrReplace(localObject);
        }
    }

    public DependencyDisappearLocalObject(OrDependency dependencyBean) {
    ...
    public static DependencyDisappearLocalObject createDependencyDisappearLocalObject(Dependency dependsOn) {
        if ("org.celstec.arlearn2.beans.dependencies.ActionDependency".equals(dependsOn.getType())) {
            return new DependencyDisappearLocalObject((ActionDependency)dependsOn);
        } else if ("org.celstec.arlearn2.beans.dependencies.AndDependency".equals(dependsOn.getType())) {
            return new DependencyDisappearLocalObject((AndDependency) dependsOn);
        } else if ("org.celstec.arlearn2.beans.dependencies.OrDependency".equals(dependsOn.getType())) {
            return new DependencyDisappearLocalObject((OrDependency) dependsOn);
        }  else if ("org.celstec.arlearn2.beans.dependencies.TimeDependency".equals(dependsOn.getType())) {
            return new DependencyDisappearLocalObject((TimeDependency) dependsOn);
        } else if ("org.celstec.arlearn2.beans.dependencies.ProximityDependency".equals(dependsOn.getType())) {
            return new DependencyDisappearLocalObject((ProximityDependency) dependsOn);
        }
        return null;
    }
    ...
    public boolean recursiveEquals(Dependency disappearOn) {
        if ("org.celstec.arlearn2.beans.dependencies.ActionDependency".equals(disappearOn.getType())) {
            if (ACTION_DEPENDENCY == getType()) return recursiveEquals((ActionDependency) disappearOn);
        } else if ("org.celstec.arlearn2.beans.dependencies.AndDependency".equals(disappearOn.getType())) {
            if (AND_DEPENDENCY == getType()) return recursiveEqualsAndOr((BooleanDependency) disappearOn);
        } else if ("org.celstec.arlearn2.beans.dependencies.OrDependency".equals(disappearOn.getType())) {
            if (OR_DEPENDENCY == getType()) return recursiveEqualsAndOr((BooleanDependency) disappearOn);
        } else if ("org.celstec.arlearn2.beans.dependencies.TimeDependency".equals(disappearOn.getType())) {
            if (TIME_DEPENDENCY == getType()) return recursiveEquals((TimeDependency) disappearOn);
        } else if ("org.celstec.arlearn2.beans.dependencies.ProximityDependency".equals(disappearOn.getType())) {
            if (PROXIMITY_DEPENDENCY == getType()) return recursiveEquals((ProximityDependency) disappearOn);
        }
        return false;
    }
    ...
    public long disappearAt(RunLocalObject run) {
        switch (getType()) {
            case ACTION_DEPENDENCY:
    ...
    private long orSatisfiedAt(RunLocalObject run) {
        ...
        for (DependencyDisappearLocalObject dependencyLocalObject: this.getChildDeps()) {
            long localMin = dependencyLocalObject.disappearAt(run);
            if (localMin != -1) minTime = Math.min(minTime, localMin);
        }
        if (minTime == Long.MAX_VALUE) return Long.MAX_VALUE;
        return minTime;
    }
    ...
    private long andSatisfiedAt(RunLocalObject run) {
        long maxTime = 0;
        for (DependencyDisappearLocalObject dependencyLocalObject: this.getChildDeps()) {
            long localMax = dependencyLocalObject.disappearAt(run);
    ...
    private long actionSatisfiedAt(RunLocalObject run) {
        ...
        if (minSatisfiedAt == Long.MAX_VALUE) minSatisfiedAt = Long.MAX_VALUE;
        return minSatisfiedAt;
    }

    private long timeSatisfiedAt(RunLocalObject run) {
        if (getChildDeps().isEmpty()) return -1;
        long satisfiedAt = getChildDeps().get(0).disappearAt(run);
        if (satisfiedAt == Long.MAX_VALUE) return Long.MAX_VALUE;
