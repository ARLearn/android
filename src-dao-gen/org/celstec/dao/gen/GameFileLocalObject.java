...
public class GameFileLocalObject {
    ...
    public static GameFileLocalObject getGameFileLocalObject(Long gameId, String path) {
        if (gameId == null || path == null) return null;
        QueryBuilder<GameFileLocalObject> qb = DaoConfiguration.getInstance().getGameFileDao().queryBuilder();
        qb.where(
                qb.and(
                        GameFileLocalObjectDao.Properties.GameId.eq(gameId)
                        ,
                        GameFileLocalObjectDao.Properties.Path.eq(path))
        );
    ...
    public static Drawable getDrawable(Context ctx, Long gameId, String path){
        try {
